...
const QuickStats = ({ savedTrips = [] }) => {
  // Shared with /profile and /trips so the same label can't show different
  // numbers on different pages. "Visited" counts completed trips only.
  const { upcomingCount, citiesVisited, daysTraveled } = getTripStats(savedTrips);

  const stats = [
    { label: "Upcoming Trips", value: String(upcomingCount), icon: CalendarIcon },
